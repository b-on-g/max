...
| `STAFF_SECRET` | Секрет приглашения: ссылка `https://max.ru/<бот>?start=staff_<секрет>` делает открывшего диспетчером. Дальше диспетчеры добавляют коллег в приложении по коду сотрудника из профиля |
...
6. Открыть приложение как диспетчер: по ссылке-приглашению с `STAFF_SECRET`, локально
   `#!start=staff_<секрет>`, либо `#!user=7` при `UK_STAFF=7`. Раздел «Диспетчер» показывает
   все заявки со сменой статуса, QR-коды домов, приглашение коллег и форму новости. Добавить
   коллегу можно и по коду сотрудника из его профиля, доверие идёт цепочкой от ключа бота.
7. API организации:
