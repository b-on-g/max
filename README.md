...
В диалоге бот отвечает на команды `/start`, `/help`, `/id` и на сообщения со словами
«заявка», «авария», «проблема», «жалоба». В групповом чате он молчит, пока его не
позовут: отвечает только на команды, упоминание `@бот` и те же слова. При добавлении в
